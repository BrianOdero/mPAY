<?php
  ...
  if(isset($_POST['book_vehicle'])) {
    ...
    $u_car_days = $_POST['u_car_days'];
    $u_car_book_status = $_POST['u_car_book_status'];

    $query = "UPDATE tms_user SET u_car_type=?, u_car_bookdate=?, u_car_regno=?, u_car_days=?, u_car_book_status=? WHERE u_id=?";
    $stmt = $mysqli->prepare($query);
    $rc = $stmt->bind_param('sssssi', $u_car_type, $u_car_bookdate, $u_car_regno, $u_car_days, $u_car_book_status, $u_id);
    $stmt->execute();
    if($stmt) {
      $succ = "Booking Submitted. Please proceed to pay";
    } else {
          ...
          <script>
            setTimeout(function () {
              swal("Success!", "<?php echo $succ; ?>!", "success")
                .then(function() {
                  window.location.href = "payment-screen.php";
                });
            }, 100);
          </script>
        <?php } ?>
          ...
          <div class="card-body">
            ...
            <form method="POST">
              <div class="form-group">
                <label for="carType">Vehicle Category</label>
                <input type="text" value="<?php echo $row->v_category;?>" readonly class="form-control" name="u_car_type">
              </div>
              <div class="form-group">
                <label for="carRegno">Vehicle Registration Number</label>
              ...
              <div class="form-group">
                <label for="carDays">Number of Days</label>
                <input type="number" class="form-control" name="u_car_days" required min="1" placeholder="Enter number of days">
              </div>
